...
describe('buildDetailedReportHtml', () => {
  ...
  it('includes income line items with source and amount', () => {
    const incomeEntries = [
      {
        id: 'i1',
        user_id: 'u1',
    ...
    expect(html).toContain('DoorDash weekly payout');
    expect(html).toContain('$380.00');
  });

  it('includes business profit in the summary table', () => {
    const html = buildDetailedReportHtml({
      title: 'Test Report',
      rangeLabel: '2026',
      rollups,
    ...
    expect(html).toContain('-$20.00'); // totals.businessProfit fixture is -20
  });
});
