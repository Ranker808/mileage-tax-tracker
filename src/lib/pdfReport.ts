...
function incomeDetailRows(income: Income[], ventureById: Map<string, Venture>): string {
  if (income.length === 0) {
    return '<div class="empty">No income logged in this range.</div>';
  }
  const rows = [...income]
    .sort((a, b) => (a.date < b.date ? -1 : 1))
    .map(
      (entry) => `
        <tr>
          ...
          <td>${escapeHtml(ventureById.get(entry.venture_id)?.name ?? 'Unknown')}</td>
          <td>${escapeHtml(entry.source)}</td>
          <td>${entry.notes ? escapeHtml(entry.notes) : ''}</td>
          <td class="num strong">${formatCurrency(entry.amount)}</td>
        </tr>`
    )
    .join('');
  return `
 ...
 * plus full trip-, expense-, and income-level line items -- everything
 * the CSV export has, in one printable file. */
export function buildDetailedReportHtml(params: {
  ...
  const { title, rangeLabel, rollups, totals, trips, expenses, income, ventureById } = params;
    ...
    .map(
      (r) => `
        <tr>
          ...
          <td class="num">${formatCurrency(r.netDeductible)}</td>
          <td class="num">${formatCurrency(r.totalIncome)}</td>
          <td class="num strong">${formatCurrency(r.businessProfit)}</td>
        </tr>`
    )
    .join('');
  ...
  return `
    <html>
      ...
      <body>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="num">Business Profit</th>
            </tr>
          </thead>
          ...
          <tfoot>
            <tr>
              ...
              <td class="num">${formatCurrency(totals.totalIncome)}</td>
              <td class="num">${formatCurrency(totals.businessProfit)}</td>
            </tr>
          </tfoot>
        </table>
        ...
        <h2>Income</h2>
        ${incomeDetailRows(income, ventureById)}

        <div class="footer">Generated by Mileage &amp; Tax Tracker. Net Deductible = mileage deduction + logged expenses (a tax figure). Business Profit = income - expenses (actual cash flow, excludes the mileage deduction).</div>
      </body>
    </html>`;
}
